...
name: cram-engine
metadata:
  short-description: AI考前速成引擎
description: 期末速成引擎——基于SM-2间隔复习、知识图谱拓扑排序、四阶段闭环（诊断→拆解→讲授→检题→补漏→复习）的AI学习系统。支持摄入往年真题、教材重点、老师题目，自动建图索引，按依赖关系和权重调度学习路径。支持中英文双语教学。硬性状态机确保不跳阶段、不逃补漏、不错必考点。
---
